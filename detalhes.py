...
st.set_page_config(page_title="RIFAnalyst - Inteligência Total", layout="wide")
...
from config import MAPA_ABAS_REGRAS, CHAVES_TEXTO, ABAS_REDE

# --- INICIALIZAÇÃO DO ESTADO ---
if "datasets" not in st.session_state:
    st.session_state["datasets"] = {}
if "tabela_documentos" not in st.session_state:
    st.session_state["tabela_documentos"] = pd.DataFrame(
        columns=["cpf_cnpj", "nome_razao_social"]
    )

# --- SIDEBAR: CARGA DE DADOS ---
st.sidebar.header("📁 Carga de Dados")
uploaded_file = st.sidebar.file_uploader(
    "Upload Planilha Excel (.xlsx)", type=["xlsx"]
)

if uploaded_file:
    if st.sidebar.button("📥 Importar e Mapear Dados"):
        with st.spinner("Processando abas e vinculando identidades..."):
            try:
                ...
                for nome_real in excel_obj.sheet_names:
                    df_temp = pd.read_excel(
                        uploaded_file, sheet_name=nome_real, nrows=0
                    )
                    dtype_map = {
                        c: str
                    ...
                    df = pd.read_excel(
                        uploaded_file, sheet_name=nome_real, dtype=dtype_map
                    )
                    st.session_state["datasets"][nome_real] = df

                # Criar Tabela Mestra de Identidades (Aba Comunicações)
                aba_com = next(
                    (
                        n
                        for n in st.session_state["datasets"].keys()
                        if "COMUNICA" in normalizar(n)
                    ),
                    None,
                )
                if aba_com:
                    df_c = st.session_state["datasets"][aba_com]
                    ident = []
                    ...
                    st.session_state["tabela_documentos"] = (
                        pd.DataFrame(ident)
                        .drop_duplicates("cpf_cnpj")
                    )

                st.sidebar.success(
                    f"Sucesso! {len(st.session_state['datasets'])} abas carregadas."
                )
            except Exception as e:
                st.sidebar.error(f"Erro na importação: {e}")

# --- INTERFACE DE FILTROS ---
if st.session_state["datasets"]:
    c1, c2 = st.columns(2)
    with c1:
        if not st.session_state["tabela_documentos"].empty:
            opcoes = (
                st.session_state["tabela_documentos"]
                .apply(
    ...
    # --- REDE PESSOAS/EMPRESAS ↔ COMUNICAÇÕES (PyVis, filtrada por alvos E IDs) ---
        # --- REDE PESSOAS/EMPRESAS ↔ COMUNICAÇÕES (PyVis, filtrada por alvos e/ou IDs) ---
    edges = []
    nodes_docs = set()
    nodes_ids = set()
    ...
    if docs_busca_rede:
        ...
        for nome_aba, df in st.session_state["datasets"].items():
            aba_norm = normalizar(nome_aba)
            ...
            if col_doc_aba:
                col_doc_real = next(
                    (
                        c
                        for c in df.columns
                        if normalizar(c) == normalizar(col_doc_aba)
                    ),
                    None,
                )
            if not col_doc_real:
    ...
    if edges:
        ...
        mapa_nome = {}
        if not st.session_state["tabela_documentos"].empty:
            for _, r in st.session_state["tabela_documentos"].iterrows():
                doc = str(r["cpf_cnpj"])
        ...
        for nome_aba, df in st.session_state["datasets"].items():
                ...
                if col_mapeada:
                    col_real = next(
                        (
                            c
                            for c in df.columns
                            if normalizar(c) == normalizar(col_mapeada)
                        ),
                        None,
                    )
            ...
            if not df_res.empty:
                ...
                with st.expander(
                    ...
                    if aba_norm == normalizar("Recursos Federais"):
                        st.markdown(
                    ...
                    if aba_norm == normalizar("Envolv. emendas parl."):
    ...
    with st.expander(
        ...
        if aba_escolhida:
            st.dataframe(
                st.session_state["datasets"][aba_escolhida],
                width='stretch',
            )
