...
# Mapeamento de idOcorrencia e Ocorrencia
#
OCORRENCIA_MAP = {
...
# Mapeamento de idOcorrencia para nível de criticidade (Score de Risco)
CRITICIDADE_MAP = {
    # --- CRÍTICO (10 pontos) ---
...
def analyze_suspicious_patterns(_df_display, _df_ocorrencias, _df_comunicacoes, _df_envolvidos):
    ...
    # --- NOVO: Análise Baseada no ID da Ocorrência e Criticidade ---
    # Cruzamos as ocorrências do arquivo original com o mapa de criticidade
    if not df_ocorrencias_local.empty:
        # Garantir que o ID seja string para o mapeamento
        df_ocor_temp = df_ocorrencias_local.copy()
        df_ocor_temp['idOcorrencia'] = df_ocor_temp['idOcorrencia'].astype(str).str.split('.').str[0]
        
        # Filtramos apenas ocorrências que estão no nosso mapa de criticidade
        df_hits = df_ocor_temp[df_ocor_temp['idOcorrencia'].isin(CRITICIDADE_MAP.keys())]
        
        # Para cada ocorrência identificada, vinculamos aos envolvidos daquela comunicação
        for _, ocor in df_hits.iterrows():
            idx = ocor['Indexador']
            id_cod = str(ocor['idOcorrencia']).split('.')[0] # Remove o .0 se houver
            desc_ocor = ocor['Ocorrencia']
            nivel = CRITICIDADE_MAP.get(id_cod, 'Moderado') # Busca no novo mapa
            
            # Buscamos quem são os envolvidos neste Indexador no df_display (filtrado)
